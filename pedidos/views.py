...
from .schemas import (
    carrito_response,
    carrito_request,
    detalle_carrito_response,
    detalle_carrito_request,
...
class DetallePedidoViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        detalle = (
            serializer.save()
        )  # Guardamos el detalle (precios y ofertas se calculan automáticamente)
...
class PedidoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            if (
                hasattr(self.request.user, "rol")
                and self.request.user.rol
                and self.request.user.rol.nombre == "Administrador"
            ):
    ...
    def calificar(self, request, pk=None):
        ...
        if calificacion is None:
            return Response(
                {"detail": "Debe proporcionar una calificación."},
        ...
        return Response(
            {"detail": "Calificación actualizada con éxito."}, status=status.HTTP_200_OK
        )

    @action(
        detail=False,
        methods=["get"],
        url_path="combinaciones-ml",
        permission_classes=[],
    )
    def combinaciones_ml(self, request):
        ...
        for pedido in pedidos:
            ...
            for target_idx, target_product in enumerate(productos):
                ...
                input_products = [
                    p for idx, p in enumerate(productos) if idx != target_idx
                ]
        ...
        if format_type.lower() == "csv":
            ...
            response["Content-Disposition"] = (
                'attachment; filename="recomendaciones_ml.csv"'
            )
            ...
            for item in resultado:
                ...
                target_json = json.dumps(
                    [item["target"]]
                )  # Convertir a lista para mantener formato consistente

                writer.writerow([input_json, target_json])
    ...
    def descargar_ml_csv(self, request):
        ...
        for pedido in pedidos:
            ...
            for target_idx, target_product in enumerate(productos):
                ...
                resultado.append({"input": input_products, "target": target_product})
...
class CarritoViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        request_body=carrito_request, responses={201: carrito_response}
    )
    ...
    def obtener_carrito_activo(self, request):
        ...
        # Actualizar precios de ofertas automáticamente al obtener el carrito
        productos_actualizados = carrito.actualizar_precios_ofertas()

        serializer = self.get_serializer(carrito)
        response_data = serializer.data

        # Agregar información sobre actualizaciones
        if productos_actualizados > 0:
            response_data["actualizaciones"] = {
                "productos_actualizados": productos_actualizados,
                "nota": "Precios actualizados según ofertas vigentes",
            }
    ...
    def convertir_a_pedido(self, request, pk=None):
        ...
        # Información sobre ofertas antes de convertir
        ahorro_total = carrito.calcular_ahorro_total()
        productos_con_oferta = carrito.detalles.filter(
            is_active=True, descuento_oferta__gt=0
        ).count()

        # Actualizar precios antes de convertir
        productos_actualizados = carrito.actualizar_precios_ofertas()
        ...
        return Response(
            {
                ...
                "pedido_id": pedido.id,
                "resumen_ofertas": {
                    "ahorro_por_ofertas": str(ahorro_total),
                    "productos_con_oferta": productos_con_oferta,
                    "productos_actualizados": productos_actualizados,
                    "total_final": str(pedido.total),
                },
            }
        )

    @swagger_auto_schema(
        responses={200: '{"mensaje": "Carrito vaciado exitosamente."}'}
    )
    ...
    @swagger_auto_schema(
        responses={200: '{"mensaje": "Precios del carrito actualizados exitosamente."}'}
    )
    @action(detail=True, methods=["post"], url_path="actualizar-precios")
    def actualizar_precios(self, request, pk=None):
        """
        Actualiza los precios de todos los productos del carrito según las ofertas vigentes.
        Útil cuando las ofertas cambian después de agregar productos al carrito.
        """
        try:
            carrito = self.get_object()
            ...
            # Actualizar precios de todos los detalles activos
            productos_actualizados = carrito.actualizar_precios_ofertas()
            ...
            return Response(
                {
                    "mensaje": f"Precios del carrito actualizados exitosamente. {productos_actualizados} productos actualizados.",
                    "productos_actualizados": productos_actualizados,
            ...
            return Response(
                {"error": f"Error al actualizar precios: {str(e)}"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def obtener_recomendaciones(self, request, pk=None):
        ...
        try:
            ...
            producto_ids = list(
                carrito.detalles.filter(is_active=True).values_list(
            ...
            try:
                ...
                productos_recomendados = Producto.objects.filter(
                    id__in=productos_recomendados_ids, is_active=True
                )
...
class DetalleCarritoViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        request_body=detalle_carrito_request, responses={201: detalle_carrito_response}
    )
    def create(self, request, *args, **kwargs):
        return super().create(request, *args, **kwargs)
        ...
        request_body=serializers.Serializer(
            {
                "cantidad": serializers.IntegerField(
                    help_text="Nueva cantidad del producto (total deseado, no incremento)"
                )
            }
        ),
    ...
    def get_queryset(self):
        ...
        return DetalleCarrito.objects.filter(
            carrito__usuario=self.request.user, carrito__activo=True, is_active=True
        )
    ...
    def perform_create(self, serializer):
        ...
        # Al crear un detalle, automáticamente se detectarán y aplicarán ofertas
        serializer.save(carrito=carrito)

    @swagger_auto_schema(
        responses={200: '{"mensaje": "Producto eliminado del carrito exitosamente."}'}
    )
    ...
    def eliminar_producto(self, request, pk=None):
        ...
        try:
            ...
            tenia_oferta = detalle.tiene_oferta_aplicada()
            ahorro_perdido = detalle.get_ahorro_total_oferta() if tenia_oferta else 0
            ...
            if tenia_oferta:
                response_data["oferta_perdida"] = {
                    "nombre_oferta": detalle.nombre_oferta,
                    "ahorro_perdido": str(ahorro_perdido),
